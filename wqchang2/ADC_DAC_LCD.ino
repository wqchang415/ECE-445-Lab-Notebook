...
#define SWITCH_PIN_0 47
#define SWITCH_PIN_1 48
...
void pitchShiftTask(void * parameter) {
  ...
  int switchState_previous = HIGH;
  int modeState_previous = HIGH;

  generateHanningWindow();
  generateSynthesisWindow();

  lcd.setCursor(0, 0);
  lcd.print("Pitch Shifter OFF   ");
  lcd.setCursor(0, 1);
  lcd.print("Mode: Down");

  while(1) {
    int switchState = digitalRead(SWITCH_PIN_0);
    int modeState = digitalRead(SWITCH_PIN_1);
    // esp_task_wdt_reset();
    while(input_block_read_ready){
      ...
      if (switchState == HIGH) {
        ...
        // lcd.print("Pitch: ");
        // lcd.setCursor(7, 1);
        // lcd.print(global_semitone_shift);
      } else {
        ...
        if (distance > 1 && distance <= 10) { 
          if (modeState == HIGH) {
            global_semitone_shift = map(distance, 0.0f, 10.0f, -1.50f, 0.00f);
          } else {
            global_semitone_shift = map(distance, 0.0f, 10.0f, 1.50f, 0.00f);
          }
          // lcd.setCursor(0, 0);
      ...
      if(switchState_previous != switchState) {
        if (switchState == HIGH) {
          lcd.setCursor(14, 0);
          lcd.print("OFF");
        } else {
      ...
      if(modeState_previous != modeState) {
        if (modeState == HIGH) {
          lcd.setCursor(6, 1);
          lcd.print("Down");
        } else {
      ...
      switchState_previous = switchState;
      modeState_previous = modeState;
    }
  }
}
...
void setup() {
  ...
  Wire.begin(11, 10);             // SDA = GPIO 11, SCL = GPIO 10
